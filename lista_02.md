...
Se (idade < 16) Imprima("Não pode votar!");

senão se (idade >= 16 E idade < 18) Imprima("Voto facultativo!");

senão Imprima("Voto obrigatório");
...
Classe Retangulo herda FormaGeometrica:
    ...
    Método Construtor(base, altura, area)
        Definição dos valores dos atributos base, altura e area com os valores passados como parâmetro.
    
    Método CalcularArea(base, altura, area):
        area = base * altura
...
Classe Circulo herda FormaGeometrica:
    ...
    Método Construtor(cor, pi, raio, area)
        Definição dos valores dos atributos cor, pi, raio, area com os valores passados como parâmetro.

    Método CalcularArea(pi, raio)
        area = pi * raio * raio
```
...
var velocidadeMaxima = x //Definindo uma velocidade máxima
var distancia = y //deifinindo a distância a ser percorrida pelo carro
var tempoMaximo = distancia / velocidadeMaxima //Definindo um tempo máximo com base na velocidade máxima
var tempo = 0 //definido o tempo, que se inicia em 0
var velocidadeInicial = 0 //definindo a velocidade inicial, que se inicia com 0 pois o carro começa em repouso
var aceleracao = constante //definindo a aceleração como constante
var distanciaPercorrida = 0 //definindo a distância percorrida pelo carro, que inicialmente é 0

//Simulando uma corrida
enquanto (distanciaPercorrida < distancia E tempo < tempoMaximo): //verifica se a distância final não foi atingida e se o tempo ainda está dentro do limite estabelecido.
    se (velocidadeInicial < velocidadeMaxima): //verifica se a velocidade máxima já foi atingida.
        velocidadeInicial = velocidadeInicial + aceleracao * tempo
    senao: //se a velocidade maxima já foi atingida, a velocidade para de aumentar e fica constante
        velocidadeInicial = velocidadeMaxima
    
    distanciaPercorrida = distanciaPercorrida + velocidadeInicial * tempo //incrementando a distância que já foi percorrida

    tempo = tempo + 1 //incrementando o tempo decorrido

se (distanciaPercorrida >= distancia): // verifica se o carro atingiu a distância máxima a ser percorrida
    tempoCorrida = tempo //se o carro alcançou a "chegada", então o tempo de corrida equivale ao tempo atual.
senao: // se o carro parou antes da diatância máxima, é porque o tempo máximo de corrida foi atingido antes disso
    tempoCorrida = tempoMaximo //define o tempo de corrida como o tempo máximo permitido para a corrida

Imprimir tempoCorrida //coloca na tela o tempo decorrido
```
...
Função MultiplicacaoDeMatrizes(matrizA, matrizB):
    ...
    //verifica se a multipliacação das matrizes existe
    ...
    matrizResultado <- novaMatriz(linhasA, colunasB) preenchida com zeros


    //MULTIPLICAÇÃO DAS MATRIZES

    //iteração nas linhas de A
    Para i de 0 até linhasA-1 faça:
        //iteração nas colunas de B
        Para j de 0 até colunasB-1 faça:
            //ietração nas colunas de B
            Para k de 0 até colunasA-1 faça:
                //calculo do item ij da matriz resultado
                matrizResultado[i][j] <- matrizResultado[i][j] + matrizA[i][k] * matrizB[k][j]

    Retornar matrizResultado //Retorna o resultado da matriz resultado da multiplicação da matriz A pela matriz B
```
